#!/bin/bash
# 30-step smoke for v18.4 W3-P2 (llpips load path is NEW code — must verify
# the metric loads, fires nonzero, and memory fits before the real run).
...
EXP=$ROOT/experiments/train_inpainting_diffusion/smoke_v18w4_p2
LOG=$ROOT/train_logs/smoke_v18w4_p2.log
exec env CUDA_VISIBLE_DEVICES=0 PYTHONUNBUFFERED=1 \
  ...
  smoke.check=True smoke.fix_timestep=100 log.val_interval=100 log.save_interval=100000 log.image_interval=10 \
  >> "$LOG" 2>&1
