...
def retrieve_timesteps(
    ...
    **kwargs,
    ...
    if not hasattr(scheduler, 'sigmas_cache'):
        assert scheduler.sigmas.numel() == scheduler.config.num_train_timesteps + 1
...
class StableDiffusionXLInvEnhancePipeline(
    DiffusionPipeline,
    ...
    def prepare_latents(
        self, image, timestep, batch_size, num_images_per_prompt, dtype, device, add_noise=True,
    ...
    def __call__(
        ...
        **kwargs,
        ...
        timesteps, num_inference_steps = retrieve_timesteps(
            self.scheduler, timesteps, device,
        )
        ...
        latents = self.prepare_latents(
            ...
            device, add_noise=True, noise=noise, generator=generator,
        )
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if getattr(self, 'intermediate_noise_predictor', None) is not None and i + 1 < len(timesteps):
                    t_next = timestep[i+1]
                    with torch.amp.autocast('cuda'):
                        noise = intermediate_noise_predictor(
                            image_lq, t_next, center_input_sample=True
                        )
