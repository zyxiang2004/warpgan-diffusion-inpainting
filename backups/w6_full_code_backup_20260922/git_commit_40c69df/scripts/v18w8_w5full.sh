#!/bin/bash
# =============================================================================
# v18.8 W5-FULL (GPU 0): HF conditioning + prior preservation, FROM SCRATCH.
#   + hf.enable=True: AnyDoor Sobel HF map of mirror photo → warp → 3ch extra
#     BrushNet conditioning (5→8ch, zero-init = safe from-scratch)
#   + prior_preserve.freq=10: DreamBooth-style, every 10th step teach
#     'photo condition → photo output' (prevent prior erosion)
#   + InvSR-calibrated supervision (latent LPIPS + small adv + UNet-D)
#   + Structure: W3-P (pass1 with anchor — mid-adjudication winner)
#   + Inference: SDEdit-300 + prompt (user-confirmed domain fixes)
# Control arms: W5-HF (GPU 1, HF only), W3-P (GPU 2, running to 100K)
# =============================================================================
ARM=w5full
GPU=${GPU:-0}
ROOT=/home/xzy/warpgan-diffusiong-inpainting/warpgan-diffusion-inpainting-main
EXP=$ROOT/experiments/train_inpainting_diffusion/v18w8_${ARM}_100k
LOG=$ROOT/train_logs/v18w8_${ARM}.log
...
if [ -n "$1" ] && [ "$1" = "resume" ]; then
  CKPT=$(ls -1 $ROOT/experiments/train_inpainting_diffusion/*_v18w8_${ARM}_100k/checkpoints/*.pt 2>/dev/null | sort | tail -1)
  if [ -n "$CKPT" ]; then EXTRA="checkpoint_path='$CKPT'"; echo "[v18w8-$ARM] resume from $CKPT at $(date)" >> "$LOG"; fi
fi
exec env CUDA_VISIBLE_DEVICES=$GPU PYTHONUNBUFFERED=1 \
  ...
  hf.enable=True hf.thresh=50.0 \
  prior_preserve.freq=10 \
  prompt.preset=invsr \
  infer.t_start=300 \
