...
def project(
        ...
        *,
        ...
        use_wandb=False,
        initial_w=None,
        image_log_step=400, # global_config.image_rec_result_log_snapshot,
        w_name: str,
        first_inv_lr=5e-3,
):
   ...
   # print('######',pose.shape,target.shape)
    ...
    # start_w = initial_w.cpu().numpy() if initial_w is not None else w_avg

    if initial_w is None:
        start_w = np.repeat(w_avg, G.backbone.num_ws, axis=1)
    else:
        start_w = initial_w
   ...
   # pose_symmetry = pose
    ...
    for step in tqdm(range(num_steps), desc="Opt w for " + w_name):
