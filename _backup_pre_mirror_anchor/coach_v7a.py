    ...
    def _route_trainable_parameters(self):
        ...
        for name, proc in self.denoising_unet.attn_processors.items():
            ...
            if proc.wplus_scale is not None:  # attn2 -> W+ branch
                ...
                # v7a: mirror gets its OWN K/V (source-separating channel,
                # lr = brushnet's — the v2 lesson: 5e-6 cannot move these in
                # 10K steps). Softmax competition + shared O stay unchanged.
                if self.use_mirror:
    ...
    def _build_novel_view(self, x, depth, c, c_novel, y_hat_novel, depth_novel,
        ...
        with torch.no_grad():
            ...
            anchor = inv_warp * vis_eff \
                ...
                + y_hat_novel * blind
            # condition: orig hybrid contract (EG3D fills the hole)
        ...
        while self.global_step < int(self.opts.max_steps):
            ...
            if self.global_step % int(self.opts.log.board_interval) == 0:
                ...
                if self.mirror_attention_procs:
                    # v7 soft-retrieval gauge: mean softmax mass on mirror
                    # tokens (measured, not parameterized). ~0.5 at init when
                    # mirror tokens equal primary tokens in count; the TRAINED
                    # trend is the signal (up = evidence being used).
                    ...
                    if shares:
                        ...
                        metrics['mirror_attn_share'] = sm
                        self.logger.add_scalar('mirror_attn_share', sm, self.global_step)
