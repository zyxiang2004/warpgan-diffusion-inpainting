...
class Coach:
    ...
    def _forward_real(self, b, t_override=None):
        ...
        # ---- pass 1 (novel): weak eps anchor, NO pixel supervision ----
        warp_img, vis_mask, _ = self.warper.forward_warp(img1=x, depth1=depth, c1=c, c2=c_novel)
        mask = 1.0 - vis_mask                      # raw splatting mask (no erosion)
        cond1 = warp_img * (1.0 - mask) + y_hat_novel * mask   # orig hybrid=True
        # The anchor is a supervision TARGET — gradients never flow through it
        # (the eps target is noise; the anchor only defines x0). Building it
        # under no_grad keeps the inverse-warp intermediate tensors off the
        # autograd bookkeeping and lowers peak memory.
        with torch.no_grad():
            ...
            # v5 mirror-anchor (first principles, teacher ④'s "warp it once and
            # take a loss against the original"): supervision coverage follows
            # GEOMETRIC AVAILABILITY, exactly like the visible region — novel-view
            # disocclusion is visible in the MIRROR camera, so the real photo is
            # transferred there with the SAME clean grid-sample transfer (depth-
            # consistent, streak-free). Real-photo coverage replaces the EG3D
            # anchor wherever geometry allows; EG3D retreats to the truly blind
            # region at its weak 0.1 weight. No hand-tuned number is introduced:
            # the weight map IS the validity mask (supervision follows information).
            if self.mirror_anchor:
                inv_warp_m, inv_valid_m = self.warper_ext.inverse_warp(
                    img2=b['x_mirror'], depth1=depth_novel, depth2=b['depth_mirror'],
                    c1=c_novel, c2=b['c_mirror'])
                hole_eff = inv_valid_m.clamp(0, 1) * mask
                blind = (1.0 - vis_eff - hole_eff).clamp(0, 1)
                anchor = inv_warp * vis_eff + inv_warp_m * hole_eff + y_hat_novel * blind
                metrics['p1_mirror_hole_cov'] = float(
                    hole_eff.sum() / (mask.sum() + 1e-6))
                panels['inv_warp_mirror'] = inv_warp_m.detach()
            else:
                hole_eff = None
        ...
        panels.update({'warp_img': warp_img.detach(), 'cond1': cond1.detach(),
