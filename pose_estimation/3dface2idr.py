...
if __name__ == '__main__':
    ...
    for src_filename in npys:
        ...
        if mode==1:
            count = 0
            K = np.eye(3)
            #  4.2647, 0.0,   0.5, 0.0,  4.2647,   0.5, 0.0, 0.0, 1.0
            # 2985.29, 0.0, 512.0, 0.0, 2985.29, 512.0, 0.0, 0.0, 1.0
            # K[0][0] = focal
            # K[1][1] = focal
            # K[0][2] = w/2.0
            # K[1][2] = h/2.0
            ...
            c = np.concatenate((np.array(pose).flatten(), np.array(K).flatten())).tolist()
            outsAll_list.append([src_filename.replace(".npy", ".png"), c])
    ...
    outAll["labels"] = outsAll_list

    if mode == 1:
        dst = os.path.join(args.out_root, "dataset.json")  # cameras.json
