...
# https://github.com/NagabhushanSN95/Pose-Warping/blob/main/src/WarperPytorch.py
...
class Warper:
    ...
    def forward_warp(self, frame1: torch.Tensor, mask1: Optional[torch.Tensor], depth1: torch.Tensor,
        ...
        transformation1 = torch.linalg.inv(transformation1)
        ...
        ## bilinear_splatting
        warped_frame2, mask2 = self.bilinear_splatting(frame1, mask1, trans_depth1, trans_coordinates, None, is_image=True)
        warped_depth2 = self.bilinear_splatting(trans_depth1, mask1, trans_depth1, trans_coordinates, None,
                                                is_image=False)[0][:, :, 0]
    ...
    def bilinear_splatting(self, frame1: torch.Tensor, mask1: Optional[torch.Tensor], depth1: torch.Tensor,
                           trans_pos: torch.Tensor, flow12_mask: Optional[torch.Tensor], is_image: bool = False) -> \
        ...
        if flow12_mask is None:
            flow12_mask = torch.ones(size=(b, 1, h, w)).to(trans_pos)
        # trans_pos = trans_pos * h - 0.5  ####
