#!/bin/bash
# =============================================================================
# v19 W10 (GPU 0): §7.30 fix arm — anchor_clean=selective_hole.
#   visible region UNTOUCHED (copy contract restored) + hole-only
#   impulse-selective median (tau 0.07, k5). Forked from W6-A@160K —
#   SINGLE variable vs the W6-A reference.
# Preregistered 20K-step window (§7.30):
#   validator good-rate >= 50% (baseline 38%), P(G) std DOWN;
#   visible L1 worsening > 20% or grad spike > 10x -> stop.
# =============================================================================
ARM=w10
GPU=${GPU:-0}
ROOT=/home/xzy/warpgan-diffusiong-inpainting/warpgan-diffusion-inpainting-main
EXP=$ROOT/experiments/train_inpainting_diffusion/v19_w10_20k
LOG=$ROOT/train_logs/v19_w10.log
...
# train_inpainting_diffusion.py DERIVES exp_dir from the ckpt path (two levels
# up) — so a fork MUST copy the base ckpt into ITS OWN checkpoints/ dir first
# (otherwise the run writes into the base experiment, as the W10 smoke
# misfire at 2026-09-22 01:38 showed).
