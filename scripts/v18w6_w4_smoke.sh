#!/bin/bash
# 30-step smoke for v18.6 W4: verifies prompt ACTIVE, t-domain [0,300],
# latent-LPIPS ACTIVE, SDEdit val sampling at step 20, memory.
...
EXP=$ROOT/experiments/train_inpainting_diffusion/smoke_v18w6_w4
LOG=$ROOT/train_logs/smoke_v18w6_w4.log
exec env CUDA_VISIBLE_DEVICES=0 PYTHONUNBUFFERED=1 \
  /home/xzy/miniconda3/envs/warpgan/bin/python scripts/train_inpainting_diffusion.py \
  exp_dir=$EXP max_steps=30 \
  prompt.preset=invsr infer.t_start=300 \
  ...
  smoke.check=True smoke.fix_timestep=100 log.val_interval=20 log.save_interval=100000 log.image_interval=10 \
  >> "$LOG" 2>&1
