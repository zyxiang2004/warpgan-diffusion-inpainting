#!/bin/bash
# =============================================================================
# v19 W8-lite (GPU 2): §7.18 preregistered ladder step-down after the W8 x10
# incident (7.22): SAME mechanism, GENTLE dose for a pretrained G + fresh D —
#   adv x1 (InvSR-calibrated magnitude) + FM x10 (v14 gradual = the project's
#   own Stage-B lesson) + warmup 3000, forked from W6-B@160K.
# Guardrail: scripts/grad_guard.sh auto-kills at g_grad_abs>3500 sustained.
# Kill line: +15K steps tierA<=16% or visible degradation -> stop (final).
# =============================================================================
ARM=w8lite
GPU=${GPU:-2}
...
exec env CUDA_VISIBLE_DEVICES=$GPU PYTHONUNBUFFERED=1 \
  ...
  losses.x0.pixel_d.enable=True losses.x0.pixel_d.warmup=3000 \
  losses.adversarial.weight=1 losses.feature_matching.weight=10 \
