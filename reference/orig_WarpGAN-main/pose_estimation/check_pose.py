...
import shutil
pose_template = np.load('../test_imgs/00001.npy')
...
#               predict_pose = labels
 if os.path.isfile('%s/cropped_images/dataset.json'%(temp_folder)):
       with open('%s/cropped_images/dataset.json'%(temp_folder), 'r') as file:     
              labels = json.load(file)['labels']
              predict_pose = labels
