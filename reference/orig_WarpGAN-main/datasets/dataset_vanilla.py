...
class Dataset(torch.utils.data.Dataset):
    def __init__(self,
        ...
        load_depth = False,     # Load depth maps
        get_c_novel = False,
    ):
        self._name = name
        ...
        self._use_depth = load_depth
        self._use_mirror = use_mirror

        self._get_c_novel = get_c_novel
    ...
    def __getitem__(self, idx):
        ...
        if self._use_depth:
            depth = torch.from_numpy(self._load_depth(self._raw_idx[idx]).copy())
        else:
            depth = ['not loaded']

        if self._get_c_novel:
            c_novel = self._load_c_novel(self._raw_idx[idx])
        else:
            c_novel = np.array([])
        ...
        if self._use_mirror:
            return image_resized, image, torch.from_numpy(label), mirror_image_resized, mirror_image, torch.from_numpy(mirror_label), mirror_conf_map, depth, torch.from_numpy(c_novel), fname
        else:
            return image_resized, image, torch.from_numpy(label), depth, torch.from_numpy(c_novel), fname
...
class ImageFolderDataset(Dataset):
    def __init__(self,
        ...
        **super_kwargs,         # Additional arguments for the Dataset base class.
        ...
        self.depth_fnames = [f'{p.split("/")[-1][:-4]}-dpt_beit_large_512.pfm' for p in self._image_fnames]
        self.depth_root = os.path.join(self._path, 'depth_map')

        self.c_novels = [f'{p.split("/")[-1][:-4]}.npy' for p in self._image_fnames]
        self.c_novels_root = os.path.join(self._path, 'c_novel')
    ...
    def _load_c_novel(self, raw_idx):
        c_novel_path = self.c_novels[raw_idx]
        c_novel = np.load(os.path.join(self.c_novels_root, c_novel_path))
        return c_novel

    def _load_depth(self, raw_idx):
        fname = self.depth_fnames[raw_idx]
        depth, _ = read_pfm(os.path.join(self.depth_root, fname))
        return depth
