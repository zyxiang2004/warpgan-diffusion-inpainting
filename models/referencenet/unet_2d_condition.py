...
class UNet2DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def __init__(
        ...
        if norm_num_groups is not None:
            ...
            self.conv_act = None
        # self.conv_norm_out = None
        # [修复]: 加回被最后一步降维卷积层 (将 320 通道压回 4 通道)
    ...
    # 在 UNet2DConditionModel 类中新增此方法
    def extract_features(
        ...
        """
        干净、显式地在前向传播中提取多尺度特征字典（完全不依赖 PyTorch Hook）。
        走完 Down Blocks 和 Mid Block，返回包含 320, 640, 1280 等全尺度特征的字典。
        """
        # 1. 准备时间步 Embedding
        timesteps = timestep
        if not torch.is_tensor(timesteps):
        ...
        # Store all resolutions for each channel width. Several SD blocks share
        # 1280 channels but operate at different spatial scales; a single tensor
        # keyed only by channels would let the mid-block overwrite the 16x16
        # down-block feature.
        features_dict = {}

        def add_feature(tensor):
            channels = tensor.shape[1]
            bucket = features_dict.setdefault(channels, [])
            spatial_shape = tensor.shape[-2:]
            # One representative per channel/resolution is sufficient. Keeping
            # every residual from the block would waste memory without changing
            # the nearest-resolution lookup in the attention processor.
            if not any(item.shape[-2:] == spatial_shape for item in bucket):
                bucket.append(tensor)

        # 3. 顺序流经所有 Down Blocks 并拦截特征
        for downsample_block in self.down_blocks:
            ...
            # Residuals contain the pre-downsample high-resolution maps (e.g.
            # 320x64x64) that are needed by matching UNet self-attention layers.
            for feature in res_samples:
                add_feature(feature)
            add_feature(sample)

        # 4. 流经 Mid Block 并拦截最深层的特征
        if self.mid_block is not None:
        ...
        # 返回装满了 [320, 640, 1280] 各分辨率特征图的干净字典
